...
<body class="flex app-shell">
    ...
    <div class="sidebar app-sidebar p-6 flex flex-col justify-between text-white shadow-2xl z-10">
        ...
        <a href="<?= base_url($isLoggedIn ? 'logout' : '/') ?>"
            class="text-xs text-yellow-100 font-bold p-3 flex items-center gap-2 hover:bg-white/10 rounded-xl transition-all">
            <i class="fa-solid <?= $isLoggedIn ? 'fa-power-off' : 'fa-arrow-left' ?>"></i>
            <?= $isLoggedIn ? 'LOG KELUAR' : 'KEMBALI' ?>
        </a>
    </div>
    ...
    <script>
        function tunjukSeksyen(seksyen) {
            ...
            document.querySelectorAll('nav button').forEach(btn => { btn.classList.remove('active-tab'); btn.classList.add('text-yellow-100','hover:bg-white/10'); });
        ...
        async function muatAcara() {
            try {
                ...
                ['upcomingGrid','ongoingGrid','pastGrid'].forEach(id => {
                    var g = document.getElementById(id); if (g && !g.children.length) g.innerHTML = emptyHtml();
                });
        ...
        function createCard(ev, isFeatured) {
            ...
            var statusBadge = isPast ? '<span class="badge-past text-[9px] font-bold px-2 py-1 rounded-full uppercase">Tamat</span>'
                : ev.event_status === 'ongoing' ? '<span class="badge-ongoing text-[9px] font-bold px-2 py-1 rounded-full uppercase">Berlangsung</span>'
            ...
            if (limit > 0) {
                ...
                capHtml = `<div class="mt-3"><div class="flex justify-between text-[10px] mb-1"><span class="text-slate-500">Kapasiti</span><span class="font-bold ${pct>=100?'text-red-600':'text-slate-600'}">${used}/${limit}</span></div><div class="capacity-bar"><div class="capacity-fill ${barColor}" style="width:${pct}%"></div></div></div>`;
            }
            var btnHtml = isPast
        ...
        function showError(msg) {
            ['featuredGrid','allEventsGrid','upcomingGrid','ongoingGrid','pastGrid'].forEach(id => {
                var el = document.getElementById(id); if (el) el.innerHTML = `<div class="col-span-3 text-center py-12"><i class="fa-solid fa-exclamation-triangle text-4xl text-red-400 mb-3"></i><p class="text-red-400 text-sm mt-3">${msg}</p></div>`;
            });
        }
